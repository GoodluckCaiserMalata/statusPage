...
const emgDeadRoutes = ["639c29eaf02f467549a5a921"];
        ...
        gettingIssues(componentId)
            .then((issues => {
                if (issues.length <= 0) {
                    ...
                    if (emgDeadRoutes.length <= 0 || emgDeadRoutes.some(deadRoute => deadRoute !== componentId)) {
...
function gettingIssues(componentId) {
    const config = {
        method: 'get',
        url: `https://beem.hund.io/api/v1/issues?limit=100&components=${componentId}`,
        headers: {
